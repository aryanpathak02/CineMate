...
function movies() {
    ...
    const getmovies = async () => {
        try {
            let { data } = await axios.get(`/${category}/airing_today?language=en-US&page=${page}`);
        ...
        <section className="h-full bg-[#1F1E24]">
            <nav className="py-6 w-5/6 mx-auto flex items-start justify-between">
                <div className="flex items-center text-zinc-300">
                    ...
                    ></i>
                    <p className="text-lg font-semibold">Tv Show</p>
                </div>
                <Searchbar />
            </nav>
            <InfiniteScroll
            ...
            >
                <VerticleCard data={movies} type={category} />
            </InfiniteScroll>
